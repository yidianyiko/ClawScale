...
describe('admin auth routes', () => {
  ...
  it('rejects protected admin routes when the token is missing', async () => {
    const app = new Hono();
    app.route('/api/admin', adminAuthRouter);
  ...
  it('rejects the current session after an admin is disabled post-login', async () => {
    const passwordHash = await hashAdminPassword('password123');
    db.adminAccount.findUnique.mockImplementation(async ({ where }: { where: { email?: string; id?: string } }) => {
      if (where.email) {
        return {
          id: 'adm_123',
          email: 'admin@example.com',
  ...
  it('treats logout as a stateless acknowledgment and does not invalidate the JWT', async () => {
    const passwordHash = await hashAdminPassword('password123');
    db.adminAccount.findUnique.mockImplementation(async ({ where }: { where: { email?: string; id?: string } }) => {
      if (where.email) {
        return {
    ...
    await expect(logoutRes.json()).resolves.toEqual({
      ok: true,
      data: null,
    });
