...
import type { ApiResponse, Workflow, WorkflowType } from '@clawscale/shared';
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Automations the AI agent can invoke during conversations.</p>
        </div>
        {isAdmin && (
      ...
      {rows.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <p className="text-gray-500">No workflows yet. Create one to extend your bot&apos;s capabilities.</p>
          {isAdmin && (
          ...
          {rows.map((w) => (
            <div key={w.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{TYPE_LABELS[w.type]}{w.description ? ` · ${w.description}` : ''}</p>
              </div>
      ...
      {creating && isAdmin && (
        <CreateWorkflowModal onClose={() => setCreating(false)} onCreated={(w) => { setRows((p) => [w, ...p]); setCreating(false); }} />
      )}
    </div>
  );
}

function CreateWorkflowModal({ onClose, onCreated }: { onClose: () => void; onCreated: (w: Workflow) => void }) {
      ...
      <div className="card w-full max-w-lg p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input className="input" placeholder="Optional — helps the AI decide when to use this" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} />
          </div>
          <div>
            <label className="label">Type</label>
            <select className="input" value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as WorkflowType }))}>
              {(Object.entries(TYPE_LABELS) as [WorkflowType, string][]).map(([v, l]) => (
