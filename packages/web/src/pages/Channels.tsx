...
export default function Channels() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAdding(true);
    try {
      const res = await api.post<ApiResponse<ChannelRow>>('/api/channels', {
        type: addType,
        name: addName,
        config: addConfig,
      });
  ...
  async function handleConnect(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        setChannels((prev) =>
          prev.map((c) => (c.id === id ? { ...c, status: 'connected' as const, gatewayPort: res.data.gatewayPort } : c)),
        );
      }
    } finally {
  ...
  async function handleDisconnect(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        setChannels((prev) =>
          prev.map((c) => (c.id === id ? { ...c, status: 'disconnected' as const, gatewayPort: null } : c)),
        );
      }
    } finally {
      setActionLoading(null);
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div>
                ...
                <select
                  className="input"
                  value={addType}
                  onChange={(e) => { setAddType(e.target.value as ChannelType); setAddConfig({}); }}
                >
                  {Object.entries(CHANNEL_CONFIG_SCHEMA).map(([type, s]) => (
                    <option key={type} value={type}>
                      {CHANNEL_ICONS[type] ?? '🔌'} {s.label}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="button" className="btn-secondary flex-1" onClick={() => { setShowAdd(false); resetAddForm(); }}>
                  Cancel
                </button>
              </div>
            </form>
      ...
      {loading ? (
        ...
        <div className="card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1 max-w-xs">
            Connect WhatsApp, Telegram, Slack, and more to start routing messages through your AI assistant.
          </p>
          {isAdmin && (
            <button className="btn-primary mt-5" onClick={() => setShowAdd(true)}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {channels.map((ch) => (
            <div key={ch.id} className="card p-5">
              ...
              {ch.gatewayPort && (
                <p className="text-xs text-gray-400 mb-3 font-mono">gateway :{ch.gatewayPort}</p>
              )}
                ...
                <div className="flex items-center gap-2 mt-4">
                  {ch.status === 'connected' ? (
                    ...
                    <button
                      ...
                      disabled={actionLoading === ch.id || ch.status === 'pending'}
                    >
                      {actionLoading === ch.id
                        ? <Loader2 className="h-3.5 w-3.5 animate-spin" />
